...
const isAllSelected = computed(() => {
    const unread = props.auth.notifications.filter(n => !n.read_at);
...
const toggleSelectAll = () => {
    if (isAllSelected.value) {
        ...
        selectedNotifications.value = props.auth.notifications
            .filter(n => !n.read_at)
            .map(n => n.id);
    }
};
                                ...
                                <Dropdown align="right" width="48">
                                    ...
                                    <template #content>
                                        <div class="px-4 py-2 border-b bg-gray-50/50">
                                            <div class="flex items-center justify-between mb-1">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <button v-else-if="$page.props.auth.unreadNotificationsCount > 0" @click.stop="markAllAsRead" class="text-gray-400 hover:text-blue-600 transition relative group">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            ...
                                            <div v-if="props.auth.notifications.some(n => !n.read_at)" class="flex items-center gap-2">
                                                <input type="checkbox" :checked="isAllSelected" @change="toggleSelectAll" class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500 h-3 w-3 cursor-pointer" />
                                        ...
                                        <div class="max-h-96 overflow-y-auto custom-scrollbar">
                                            ...
                                            <div v-for="notif in (showAllNotifications ? $page.props.auth.notifications : $page.props.auth.notifications.slice(0, 5))" 
                                                :key="notif.id" 
